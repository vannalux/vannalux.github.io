...
function hoverbutton_pluginAppObj_18() {
    
        var container_btn = $('#pluginAppObj_18_container');
        var button = $('#pluginAppObj_18 .pluginAppObj_18-button');
    
    x5engine.boot.push(function(){
        if("Отправить заявку" != ""){
            $("#pluginAppObj_18 .div-txt").html(br("Отправить заявку"));
        }
    ...
    function fade(){
        ...
        container_btn.on('mouseenter', function() {
            ...
            var overElem = $('#pluginAppObj_18 .button-wrapper-over');
            if (overElem.length == 0) overElem = button.clone().addClass('button-wrapper-over fade').removeAttr('id').attr('id','pluginAppObj_18_overElem'). appendTo(container_btn);
            setTimeout(function() {
    ...
    function sweep(){
        ...
        container_btn.on('mouseenter', function() {
            ...
            if (overElem.length == 0) overElem = button.clone().removeAttr('id').attr('id','pluginAppObj_18_overElem').appendTo(container_btn).wrap('<div class="sweeper"></div>').addClass('button-wrapper-over sweep').removeAttr('id').attr('id','pluginAppObj_18_overElemWrapper').css('width',button.css('width')).parent(); 
            setTimeout(function() {
                overElem.addClass('animated');
            }, 10);
        }).on('mouseleave', function() {
    ...
    function rotate_under(){ 
        ...
        container_btn.on('mouseenter', function() {
            ...
            if (overElem.length == 0) button.clone().addClass('button-wrapper-over rotate-under').removeAttr('id').attr('id','pluginAppObj_18_overElem').appendTo(container_btn);
            setTimeout(function() {
